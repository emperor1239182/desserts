...
const Cart = ()=>{
    const { cart, dessert } = useContext(CartCount);
    const svg ={
    ...
    <>
    <div className="cart">
        <h3 style={{position: 'absolute', left:'0', top:'-10px', color:'hsl(14, 86%, 42%)', fontSize:'13px'}}>Your Cart ({Object.values(cart).reduce((a, b) => a + b, 0)})</h3> <br/>
        <svg xmlns="http://www.w3.org/2000/svg" width="70" height="60" fill="none" viewBox="0 0 128 128" style={svg}>
    ...
    {dessert.length === 0 ? (<p className="appear">Your added items will appear here</p>) : (
    <div className="cartList">
        {dessert.map((item, index) => (
            <div key={index}>
                <p className="cartName">{item}</p>
            </div>
        ))}
    </div>
    )
